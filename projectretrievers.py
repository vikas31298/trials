...
def _get_product_sales_table_text(month_num: int, year: int, company: str = "comp1") -> str:
    conn = psycopg2.connect(
        host="localhost",
        database="ResourcifyDB",
        user="postgres",
        password="Postgres@1"
    )
    df = pd.read_sql_query("SELECT * FROM resourcifyschema.projects", conn)
    conn.close()
...
def _get_product_sales_table(month_num: int, year: int, company: str = "comp1") -> dict:
    ...
    for _, row in df.iterrows():
        product_dict = {
            ...
            "sales_change_mom_qty": _get_project_revenue_growth_mom(month_num, year, company=company, product=row["Product Name"]),
            "sales_revenue": row["Sales Revenue ($)"],
            "sales_change_mom_revenue": _get_project_revenue_growth_mom(month_num, year, company=company, product=row["Product Name"])
        }
...
def _get_project_revenue_growth_mom(
    month_num: int,
    year: int,
    company: str = "comp1",
    category: str = "",
...
def _get_project_revenue_growth_yoy(
    month_num: int,
    year: int,
    company: str = "comp1",
    category: str = "",
...
def _get_financial_metrics_text(
    ...
    data = [
        ...
        [
            "Total Sales\nRevenue ($)",
            format_value(_get_project_revenue(month_num, year, company=company)),
            format_value(_get_project_revenue(month_num - 1, year, company=company)),
            format_value(_get_project_revenue(month_num, year - 1, company=company)),
        ],
        [
            "Sales Growth\nRate (MoM) (%)",
            format_value(_get_project_revenue_growth_mom(month_num, year, company=company)),
            format_value(_get_project_revenue_growth_mom(month_num - 1, year, company=company)),
            format_value(_get_project_revenue_growth_mom(month_num, year - 1, company=company)),
        ],
        [
            "Sales Growth\nRate (YoY) (%)",
            format_value(_get_project_revenue_growth_yoy(month_num, year, company=company)),
            format_value(_get_project_revenue_growth_yoy(month_num - 1, year, company=company)),
            format_value(_get_project_revenue_growth_yoy(month_num, year - 1, company=company)),
        ],
        [
            "Gross Margin (%)",
...
async def _get_executive_summary(month_num: int, year: int, department: str, company: str = "comp1") -> str:
    ...
    "Customer retention improved by 5% QoQ. ",
    "  Gross margin remained stable at 40%.",
    "New customer acquisition grew by 20%. ",
    " Year-over-year revenue grew by 30%. "
    ]
...
async def _get_product_sales_bullets(
   ...
   # response = await get_all_extractions(product_sales_prompt, data + campaign_context)
    response = [
        "Sales increased by 15% compared to last month | Customer retention improved by 5% QoQ",
        "Gross margin remained stable at 40% | New customer acquisition grew by 20%",
        "Year-over-year revenue grew by 30%"
    ]
...
async def _get_project_revenue_bullets(
    month_num: int, year: int, company: str = "comp1"
) -> str:
    data = _get_product_sales_table_text(month_num, year, company=company)
    campaign_context = _get_campaign_data(month_num, company=company)
...
def _get_category_sales_chart(
    ...
    df = execute_query(f"SELECT DISTINCT category FROM sales_data WHERE company = '{company}'")
    categories = df["category"].unique().tolist()

    # Get sales data for current month, last month, and same month last year
    current_sales = [
        int(_get_project_revenue(month_num, year, company=company, category=category)) for category in categories
    ]
    last_month_sales = [
        int(_get_project_revenue(month_num - 1, year, company=company, category=category))
        for category in categories
    ]
    last_year_sales = [
        int(_get_project_revenue(month_num, year - 1, company=company, category=category))
        for category in categories
    ]
